...
/**
 * A road attaches to one of eight "ports" on a city marker: a side (the 20px
 * square is 2 tiles wide) plus which half-lane it uses. `right-top` leaves the
 * right side on the upper lane; `top-left` leaves the top side on the left lane.
 */
export type Port =
...
export interface Route {
    from: string;
    to: string;
    /** Port the road leaves `from` / enters `to`. Omit to auto-face the other city. */
    fromPort?: Port;
    toPort?: Port;
}
